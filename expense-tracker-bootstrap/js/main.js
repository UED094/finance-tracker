...
const modalInput = document.getElementById("newCategory");
...
function createTransactionItemElement(transaction) {
    ...
    item.className = "list-group-item d-flex justify-content-between align-items-center border-top-0  border-start-0 border-5 mb-2";
    // item.classList.add("list-group-item d-flex justify-content-between align-items-center border-top-0  border-start-0 border-5 border-danger mb-2");
    const className = transaction.amount < 0 ? "border-danger" : "border-success";
...
function updateCategorySelect() {
    ...
    const defaultOptions = [
        ...
        {id: 1, categoryText: '+ Add New Category', categoryValue: 'new-category'},
    ];
    ...
    categories.forEach((category) => {
        const option = createCategoryOption(category.categoryText, category.categoryValue);
...
addCategoryButton.addEventListener("click", (e) => {
    e.preventDefault();
    addNewCategory();
    closeCategoryModal(); // Close the modal after adding a new category
});

/*
// Add an event listener for Modal focusing
